...
-- 10. Audience Votes Table (Live voting, 1 vote per participant per person & per public IP)
create table audience_votes (
  ...
  reg_no text,
  submitted_at timestamptz not null default now(),
  constraint uq_audience_participant_event unique (audience_id, participant_id, event_id),
  constraint uq_ip_participant_event unique (ip_address, participant_id, event_id)
);
...
create index if not exists idx_audience_votes_ip_cat on audience_votes(ip_address, category_id);
create index if not exists idx_audience_votes_device_cat on audience_votes(device_fingerprint, category_id);
create index if not exists idx_audience_votes_phone_cat on audience_votes(phone, category_id);
create index if not exists idx_audience_votes_reg_cat on audience_votes(reg_no, category_id);
